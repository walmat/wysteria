  ...
  return (
    <div className="flex flex-col items-center gap-2 text-center">
      <a href="#" className="flex flex-col items-center gap-2 font-medium">
        ...
        <span className="sr-only">{pkg.name.charAt(0).toUpperCase() + pkg.name.slice(1)}</span>
      </a>
      <h1 className="text-xl font-bold">{title}</h1>
      {step === 'verify' && (
        <FieldDescription>We sent a 6-digit code to {identifier}</FieldDescription>
      )}
    </div>
  )
})
...
const SocialButtons = memo(function SocialButtons() {
  ...
  return (
    <Field className="grid gap-4 sm:grid-cols-2">
      <Button onClick={handleAppleSignIn} variant="outline" type="button">
...
function LoginStep({ field, isSubmitting }: { field: any; isSubmitting: boolean }) {
  return (
    <>
      <Field>
                ...
                {step === 'login' ? (
                  ...
                  >
                    <form.Field name="identifier">
                      {(field) => (
                        <LoginStep field={field} isSubmitting={form.state.isSubmitting} />
                      )}
                    </form.Field>
                  </motion.div>
                ) : (
